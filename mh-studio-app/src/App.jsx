...
// ============================================================
// CONFIGURAÇÕES DO STUDIO — edite aqui
// ============================================================
const WHATSAPP = '5547999999999' // DDI + DDD + número, só dígitos
...
const SERVICOS = [
  {
    ...
    price: 'R$ 60,00',
    desc: 'Esmaltação impecável, cuticulagem perfeita e hidratação',
    accent: 'from-[#fdf8f0] to-[#f5e6cc]',
  },
  {
    id: 'gel-tips',
    name: 'Alongamento em Gel na Tips',
    price: 'R$ 89,90',
    desc: 'Alongamento resistente, natural e super brilhante',
    badge: 'MAIS PEDIDO',
    accent: 'from-[#0a1f44] to-[#1a3a6b]',
  },
  {
    id: 'perfeito',
    name: 'Alongamento Perfeito',
    price: 'R$ 95,00',
    desc: 'Técnica exclusiva Myleine para unhas finas e duráveis',
    accent: 'from-[#c9a86c] to-[#e8d5a3]',
  },
]
...
export default function App() {
  ...
  const [servico, setServico] = useState(SERVICOS[1])
  ...
  // Busca no Supabase os horários já reservados do dia escolhido
  useEffect(() => {
    let ativo = true
    setHorario(null)
    ...
    supabase
      ...
      .then(({ data, error }) => {
        if (!ativo) return
        setOcupados(error ? [] : (data || []).map((r) => r.horario.slice(0, 5)))
  ...
  async function confirmarAgendamento() {
    ...
    if (!supabase) { setConfirmado(true); return }
              ...
              <div className="mt-7 px-4 pb-4">
                ...
                <div className="space-y-3">
                  {SERVICOS.slice(0, 2).map((s) => (
                    <div key={s.id} className="rounded-[20px] bg-white p-4 flex items-center gap-4 navy-shadow border border-[#f5e6cc]/60">
                      <div className={`w-12 h-12 rounded-full bg-gradient-to-br ${s.accent} flex items-center justify-center shrink-0`}>
                        <Scissors className={`w-5 h-5 ${s.id === 'gel-tips' ? 'text-white' : s.id === 'perfeito' ? 'text-[#0a1f44]' : 'text-[#c9a86c]'}`} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="text-[13px] font-semibold text-[#0a1f44] leading-tight truncate">{s.name}</div>
                        <div className="text-[11px] text-[#0a1f44]/60 mt-0.5 line-clamp-1">{s.desc}</div>
                      </div>
                      <div className="text-right shrink-0">
                        <div className="text-[13px] font-bold text-[#0a1f44]">{s.price}</div>
          ...
          {tab === 'servicos' && (
            <div className="px-4 pt-2">
              ...
              <div className="space-y-4 pb-6">
                {SERVICOS.map((s) => (
                  <div key={s.id} className="rounded-[24px] bg-white overflow-hidden navy-shadow border border-[#f0e6d3]">
                    {s.badge && (
                    ...
                    <div className="p-5">
                      <div className="flex gap-4">
                        <div className={`w-[64px] h-[64px] rounded-[18px] bg-gradient-to-br ${s.accent} flex items-center justify-center shrink-0 relative`}>
                          <Scissors className={`w-6 h-6 ${s.id === 'gel-tips' ? 'text-white' : s.id === 'perfeito' ? 'text-[#0a1f44]' : 'text-[#c9a86c]'}`} />
                          {s.id === 'gel-tips' && <Sparkles className="w-3 h-3 text-[#e8d5a3] absolute top-2 right-2" />}
                        </div>
                        <div className="flex-1">
                          <div className="serif text-[16px] font-semibold text-[#0a1f44] leading-tight">{s.name}</div>
                          <div className="text-[12px] text-[#0a1f44]/60 mt-1.5 leading-snug">{s.desc}</div>
                          <div className="mt-3 flex items-center gap-2">
                            <Clock3 className="w-3.5 h-3.5 text-[#c9a86c]" />
                            <span className="text-[11px] text-[#0a1f44]/70">60-90 min • inclui hidratação</span>
                          </div>
                        </div>
                      </div>
                      <div className="mt-5 flex items-center justify-between">
                        <div>
                          ...
                          <div className="text-[20px] font-bold text-[#0a1f44] serif">{s.price}</div>
                        </div>
                        <button
              ...
              {!confirmado ? (
                <>
                  ...
                  <div className="rounded-[15px] bg-[#0a1f44] px-4 py-3 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-[11px] text-white/60">{servico.price}</div>
                      </div>
                    </div>
                  ...
                  <div className="mt-6 rounded-[20px] bg-white border border-[#f0e6d3] p-4 navy-shadow">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="serif text-[18px] font-bold text-[#0a1f44]">{servico.price}</span>
                    </div>
                  </div>
                ...
                <div className="pt-10 pb-6 text-center">
                  ...
                  <div className="mt-6 mx-4 rounded-[18px] bg-white border border-[#f0e6d3] p-4 text-left">
                    ...
                    <div className="text-[13px] text-[#0a1f44] font-semibold">{servico.name} • {servico.price}</div>
                ...
                <div className="relative flex gap-4">
                  ...
                  <div>
                    ...
                    <div className="mt-3 flex items-center gap-1.5 text-[11px] text-white/70">
                      <MapPin className="w-3 h-3 text-[#c9a86c]" /> Balneário Camboriú • Studio Privativo
                    </div>
                  </div>
                </div>
